...
export class Compiler {
  ...
  run() {
    ...
    this._compilation = new Compilation({
      loaders: this.config?.module?.rules || [],
      entry: this._entry,
    });
  ...
  emitFiles() {
    ...
    // 最后基于 output 生成 不能 bundle 文件
    const outputFilePath = path.join(this._output.path, this._output.filename);
    if (!fs.existsSync(this._output.path)) {
      fs.mkdirSync(this._output.path);
    }
    fs.writeFileSync(outputFilePath, createBundleCode({ modules }));
  }
}
